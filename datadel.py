...
class datadel(object):
    ...
    #def del_year_mapdata(self,year):
        ...
        #yfilename = '%s%s%s' % ('html/0/', str(year), 'map.html')
        #ylat = latdata.values
        #ylon = londata.values
        #ytext = loctext.values
        #chart.make_mapchart(ylat, ylon, ytext, yfilename, year)
    ...
    def set_month_region(self, month):
        for index in self.month_group.index:
            month_region = self.region_month1.loc[month]
            chart_title = '%s%s' % (month, '月份地区恐怖袭击数量占比图')
            filename = '%s%s%s' % ('1/', month, '_region')
    ...
    def set_year_nkill_nwound(self):
        year_nkill_nwound = []
        year_nkill_nwound1 = self.data_list.pivot_table(values=['nkill','nwound'],index='iyear',aggfunc='sum',fill_value=0,margins=True)
        ...
        chart.make_bar_linechart(year_nkill_nwound, '年度伤亡人数统计图', '年份', '死(nkill)/伤(nwound)人数', '2/year_nkill_nwound')
    ...
    def set_region_year_nkill_nwound(self):
        ...
        chart.make_bar_linechart(region_year_nkill_nwound, '地区年度伤亡人数统计图', '年份', '死(nkill)/伤(nwound)人数', '2/region_year_nkill_nwound')
    ...
    def set_region_success(self):
        region_success = []
        region_success1 = self.data_list.pivot_table(values='success',index='iyear',columns='region_txt',aggfunc='sum',fill_value=0,margins=True)
    ...
    def set_region_suicide(self):
        region_suicide = []
        region_suicide1 = self.data_list.pivot_table(values='suicide', index='iyear', columns='region_txt',aggfunc='sum',fill_value=0, margins=True)
    ...
    def set_attacktype(self):
        ...
        attacktype_group = pd.concat([self.attacktype1_group,self.attacktype2_group,self.attacktype3_group,attacktype_group1])
    ...
    def set_region_attacktype(self):
        ...
        region_attacktype_group = pd.concat([region_attacktype1_group,region_attacktype2_group,region_attacktype3_group])
    ...
    def set_region_extended(self):
        region_extended = []
        region_extended1 = self.data_list.pivot_table(values='extended', index='iyear', columns='region_txt',aggfunc='sum', fill_value=0, margins=True)
    ...
    def set_region_weaptype_group(self):
        self.chart_title = '地区袭击武器类型数量占比图'
